...
int SceneReader::callback(p_ply_argument argument) {
	...
	boost::function<int(p_ply_argument)>* f = static_cast<boost::function<int(p_ply_argument)>* >(pData);
...
void SceneReader::readScene(const char* fileName, Scene& scene){
	...
	//Create function wrapper for each callback and lock "this" and "&scene" as parameteres, so that
	//the member functions can be used as callbacks with access to the Scene object.
	boost::function<int(p_ply_argument)> v_cb = boost::bind(&SceneReader::vertex_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> t_cb = boost::bind(&SceneReader::triangle_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> m_cb = boost::bind(&SceneReader::material_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> s_cb = boost::bind(&SceneReader::sphere_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> p_cb = boost::bind(&SceneReader::plane_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> pL_cb = boost::bind(&SceneReader::pointLight_cb, this, &scene, _1);
	boost::function<int(p_ply_argument)> dL_cb = boost::bind(&SceneReader::dirLight_cb, this, &scene, _1);
	...
	if (!ply_read(ply)) throw std::exception("Error occured while reading the scene");
