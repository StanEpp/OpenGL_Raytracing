...
void SceneLoader::initialize(OpenGLRaytracer& ogl){
	...
	ShaderManager::getInstance()->getBufferVariableIndices(ogl.getCompShaderProgName(), _objectElements, oNames, _oIndices);
	ShaderManager::getInstance()->getBufferVariableIndices(ogl.getCompShaderProgName(), _materialElements, mNames, _mIndices);
	ShaderManager::getInstance()->getBufferVariableIndices(ogl.getCompShaderProgName(), _lightElements, lNames, _lIndices);

	_oBlockIndex = glGetProgramResourceIndex(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, "PrimitiveBuffer");
	_mBlockIndex = glGetProgramResourceIndex(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, "MaterialBuffer");
	_lBlockIndex = glGetProgramResourceIndex(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, "LightBuffer");

	for(int i = 0; i < _materialElements; ++i){
		glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_BUFFER_VARIABLE, _mIndices[i], 1, props2, 1, NULL, &_mOffsets[i]);
	}

	for(int i = 0; i < _objectElements; ++i){
		glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_BUFFER_VARIABLE, _oIndices[i], 1, props2, 1, NULL, &_oOffsets[i]);
	}

	for(int i = 0; i < _lightElements; ++i){
		glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_BUFFER_VARIABLE, _lIndices[i], 1, props2, 1, NULL, &_lOffsets[i]);
	}

	glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, _oBlockIndex, 1, props, 1, NULL, &_oBlockSize);
	glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, _mBlockIndex, 1, props, 1, NULL, &_mBlockSize);
	glGetProgramResourceiv(ShaderManager::getInstance()->getShaderProgramID(ogl.getCompShaderProgName()), GL_SHADER_STORAGE_BLOCK, _lBlockIndex, 1, props, 1, NULL, &_lBlockSize);
...
void SceneLoader::test(OpenGLRaytracer& ogl){
	//glGetError();

	//glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, ogl.getStorageBufferIDs()[1]);
	//glBindBuffer(GL_COPY_WRITE_BUFFER, _copyBufferIDs[1]);

	//glCopyBufferSubData(GL_SHADER_STORAGE_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, _numberOfMaterialsInShader*_mAlignOffset);

	//glBufferData(GL_SHADER_STORAGE_BUFFER, (_numberOfMaterialsInShader + 3) * _mAlignOffset, NULL, GL_STATIC_DRAW);

	//glCopyBufferSubData(GL_COPY_WRITE_BUFFER, GL_SHADER_STORAGE_BUFFER, 0, 0, 3*_mAlignOffset);

	//if(glGetError() != GL_NO_ERROR){
	//	std::cout << "shit!" <<std::endl;
	//}
	//glBindBuffer(GL_SHADER_STORAGE_BUFFER, ogl.getStorageBufferIDs()[0]);

	//void* p = malloc(_numberOfObjInShader*_oAlignOffset);// = new Primitive[_numberOfObjInShader];
	//
	//glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, _numberOfObjInShader*_oAlignOffset, (void *)(p));

	//glBufferData(GL_SHADER_STORAGE_BUFFER, (_numberOfObjInShader+12)*_oAlignOffset, p, GL_DYNAMIC_DRAW);

	//glBindBuffer(GL_SHADER_STORAGE_BUFFER, NULL);

	//free(p);
	//glCopyBufferSubData(_copyBufferIDs[0], ogl.getStorageBufferIDs()[0], 0, 0, _numberOfObjInShader*_oAlignOffset);
}
...
void SceneLoader::loadScene(Scene& scene, OpenGLRaytracer& ogl){
	...
	if(scene.numberOfObjects() != 0){
		...
		std::vector<std::pair<Triangle, int> >::iterator tIt = scene.getTriangles()->begin();
		int type = 3;
		...
		std::vector<std::pair<Sphere, int> >::iterator sIt = scene.getSpheres()->begin();
		...
		std::vector<std::pair<Plane, int> >::iterator pIt = scene.getPlanes()->begin();
		type = 2;
	...
	if(scene.getMaterials()->size() != 0){
		...
		std::vector<Material>::iterator mIt = scene.getMaterials()->begin();
	...
	if(scene.numberOfLights() != 0){
		...
		std::vector<PointLight>::iterator pIt = scene.getPointLights()->begin();
		...
		std::vector<DirectionalLight>::iterator dIt = scene.getDirectionalLights()->begin();
