...
void ShaderManager::loadShader(const char* filename, const std::string& shaderKey, GLenum type){
	...
	try{
		...
		throw std::runtime_error((std::string("Could not load file ")+filename+": "+e.what()).c_str());
	}
...
GLuint ShaderManager::getShaderID(const std::string& shaderKey) const{
	return _shaderData->getShaderID(shaderKey);
}

GLuint ShaderManager::getShaderProgramID(const std::string& shaderProgramKey) const{
...
void ShaderManager::getBufferVariableIndices(const std::string& shaderProgram, const int length, const GLchar** names, GLint* indices) const{
