...
class ShaderManager{
private:
	std::shared_ptr<ShaderBaseModel>	_shaderData;
...
public:
	...
	GLuint 		getShaderID(const std::string& shaderKey) const;
	GLuint 		getShaderProgramID(const std::string& shaderProgramKey) const;
	...
	void 		getBufferVariableIndices(const std::string& shaderProgram, const int length, const GLchar** names, GLint* indices) const;
