...
public:
	...
	_categories(new categories()), _inputFile(defaultInputFile), _outputFile(defaultOutputFile){
	}

	XMLParser() : _categories(new categories()){
	}
	...
	void add(const std::string& categorie, const std::string& key, const char* value);
